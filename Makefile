...
SELF_EXE = target/release/jimmy
SRC = $(wildcard src/**/*.rs src/*.rs) Cargo.toml

.PHONY: all
all: $(SELF_EXE) jimmy.manifest
ifeq ($(SGX),1)
all: jimmy.manifest.sgx jimmy.sig
endif
...
jimmy.manifest: jimmy.manifest.template
	gramine-manifest \
...
jimmy.manifest.sgx jimmy.sig: sgx_sign
	@:

.INTERMEDIATE: sgx_sign
sgx_sign: jimmy.manifest $(SELF_EXE)
	gramine-sgx-sign \
		--manifest $< \
		--output $<.sgx
...
test-jimmy: all
	mkdir -p store
	bash -c 'set -a && source .env && $(GRAMINE) jimmy'
